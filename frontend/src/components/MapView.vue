...
import type { MunicipalityWithCount } from '@application-finder/shared/src/types.js';
...
const municipalityColorHex: Record<string, string> = {
  zwolle: '#0047bb',
  kampen: '#1a8f3c',
...
function createMarkerIcon(color: string, count: number) {
  return L.divIcon({
...
onMounted(async () => {
  ...
  try {
    ...
    for (const m of municipalities) {
      const mData = m as MunicipalityWithCount & { lat: number; lng: number };
      if (!mData.lat || !mData.lng) continue;
