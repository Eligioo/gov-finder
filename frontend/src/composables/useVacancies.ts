...
import type { Vacancy, VacancyListResponse } from '@application-finder/shared/src/types.js';
...
export function useVacancies() {
  ...
  const filters = ref<VacancyListResponse['filters']>({
    municipalities: [],
    educationLevels: [],
    employmentTypes: [],
  });
  ...
  const municipality = computed(() => (route.query.municipality as string || '').split(',').filter(Boolean));
  const education = computed(() => (route.query.education as string || '').split(',').filter(Boolean));
  const employmentType = computed(() => (route.query.employment_type as string || '').split(',').filter(Boolean));
  ...
  function setFilter(key: string, value: string | string[] | undefined) {
    ...
    if (key !== 'page') delete newQuery.page;
  ...
  watch(
    () => route.query,
    () => {
      if (route.name === 'home') debouncedLoad();
    },
    { immediate: true },
  );
  ...
  const activeFilterCount = computed(() => {
    ...
    if (municipality.value.length) count++;
    if (education.value.length) count++;
    if (employmentType.value.length) count++;
    return count;
  });
